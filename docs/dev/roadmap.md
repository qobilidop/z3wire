...
- **Use `map` for repeated fields in proto** — Register arrays generate
  `repeated` proto fields today, which serialize every element including zeros.
  Switch to `map<uint32, RegType>` so only non-default entries are serialized.
  The C++ struct side stays as `std::array` (dense); the map is purely a
...
- **Scoped solver checks** — The push/add/check/pop pattern is repeated in every
  verification. A helper like `z3w::check(solver, constraint)` could encapsulate
  this. Trades explicitness for brevity — needs careful design.
