...
inline z3w::Ubv<8> ErrorInfoSymbolic::Pack() const {
  return z3w::concat(reserved, z3w::to_ubv1(fatal), z3w::cast<z3w::Ubv<2>>(severity), code);
}

inline ErrorInfoConcrete ErrorInfoSymbolic::ToConcrete(const z3::model& model) const {
  ErrorInfoConcrete result{};
  result.code = std::get<0>(z3w::UInt<4>::checked(static_cast<uint32_t>(model.eval(code.raw()).get_numeral_int64())));
  result.severity = std::get<0>(z3w::SInt<2>::checked(static_cast<int32_t>(model.eval(severity.raw()).get_numeral_int64())));
  result.fatal = model.eval(fatal.raw()).is_true();
  result.reserved = std::get<0>(z3w::UInt<1>::checked(static_cast<uint32_t>(model.eval(reserved.raw()).get_numeral_int64())));
  return result;
}
...
inline ErrorInfoSymbolic ErrorInfoSymbolic::FromConcrete(z3::context& ctx,
  ...
  result.fatal = (concrete.fatal ? z3w::Bool::True(ctx) : z3w::Bool::False(ctx));
...
inline StatusRegisterSymbolic StatusRegisterSymbolic::Create(z3::context& ctx,
  ...
  for (size_t i = 0; i < 4; ++i) {
    result.counters[i] = z3w::Ubv<4>(ctx, prefix + ".counters[" + std::to_string(i) + "]");
  }
...
inline StatusRegisterConcrete StatusRegisterSymbolic::ToConcrete(const z3::model& model) const {
  ...
  result.mode = std::get<0>(z3w::UInt<2>::checked(static_cast<uint32_t>(model.eval(mode.raw()).get_numeral_int64())));
  result.error = error.ToConcrete(model);
  for (size_t i = 0; i < 4; ++i) {
    result.counters[i] = std::get<0>(z3w::UInt<4>::checked(static_cast<uint32_t>(model.eval(counters[i].raw()).get_numeral_int64())));
  }
  result.reserved = std::get<0>(z3w::UInt<5>::checked(static_cast<uint32_t>(model.eval(reserved.raw()).get_numeral_int64())));
  return result;
}
...
inline StatusRegisterSymbolic StatusRegisterSymbolic::FromConcrete(z3::context& ctx,
    const StatusRegisterConcrete& concrete) {
  StatusRegisterSymbolic result;
  result.ready = (concrete.ready ? z3w::Bool::True(ctx) : z3w::Bool::False(ctx));
