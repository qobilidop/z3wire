...
inline ErrorInfoConcrete ErrorInfoConcrete::FromProto(const ErrorInfoProto& proto) {
  ...
  result.code = std::get<0>(z3w::UInt<4>::FromValue(proto.code()));
  result.severity = std::get<0>(z3w::SInt<2>::FromValue(proto.severity()));
...
inline ErrorInfoConcrete ErrorInfoSymbolic::ToConcrete(const z3::model& model) const {
  ErrorInfoConcrete result{};
  result.code = std::get<0>(z3w::UInt<4>::FromValue(static_cast<uint32_t>(model.eval(code.expr(), true).get_numeral_int64())));
  result.severity = std::get<0>(z3w::SInt<2>::FromValue(static_cast<int32_t>(model.eval(severity.expr(), true).get_numeral_int64())));
  result.fatal = z3w::Bool(model.eval(fatal.expr(), true).is_true());
  result.reserved = std::get<0>(z3w::UInt<1>::FromValue(static_cast<uint32_t>(model.eval(reserved.expr(), true).get_numeral_int64())));
  return result;
}
...
inline StatusRegisterConcrete StatusRegisterConcrete::FromProto(const StatusRegisterProto& proto) {
  ...
  result.mode = std::get<0>(z3w::UInt<2>::FromValue(proto.mode()));
  result.error = ErrorInfoConcrete::FromProto(proto.error());
  for (size_t i = 0; i < 4; ++i) {
    result.counters[i] = std::get<0>(z3w::UInt<4>::FromValue(proto.counters(i)));
  }
  return result;
}
...
inline StatusRegisterConcrete StatusRegisterSymbolic::ToConcrete(const z3::model& model) const {
  ...
  result.mode = std::get<0>(z3w::UInt<2>::FromValue(static_cast<uint32_t>(model.eval(mode.expr(), true).get_numeral_int64())));
  result.error = error.ToConcrete(model);
  for (size_t i = 0; i < 4; ++i) {
    result.counters[i] = std::get<0>(z3w::UInt<4>::FromValue(static_cast<uint32_t>(model.eval(counters[i].expr(), true).get_numeral_int64())));
  }
  result.reserved = std::get<0>(z3w::UInt<5>::FromValue(static_cast<uint32_t>(model.eval(reserved.expr(), true).get_numeral_int64())));
  return result;
}
